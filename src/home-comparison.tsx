...
const HomeComparison = () => {
  ...
  const Check = () => (
    <div className="relative flex items-center justify-center w-8 h-8 rounded-full bg-green-100 group">
      <svg className="w-5 h-5 text-green-500 transform transition-transform duration-200 group-hover:scale-110" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  const Cross = () => (
    <div className="relative flex items-center justify-center w-8 h-8 rounded-full bg-red-100 group">
      <svg className="w-5 h-5 text-red-500 transform transition-transform duration-200 group-hover:scale-110" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  const SolutionCheck = () => (
    <div className="relative flex items-center justify-center w-10 h-10 rounded-full bg-indigo-100 ring-4 ring-indigo-500/20 group">
      <svg className="w-6 h-6 text-indigo-600 transform transition-transform duration-200 group-hover:scale-110" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="divide-y divide-gray-50">
            {comparisonPoints.map((point, index) => (
              <div key={index} 
                ...
                <div className="flex justify-center">{point.traditional ? <Check /> : <Cross />}</div>
                <div className="flex justify-center">{point.freelancer ? <Check /> : <Cross />}</div>
                <div className="flex justify-center">{point.solution ? <SolutionCheck /> : <Cross />}</div>
              </div>
            ))}
          </div>
        </div>
